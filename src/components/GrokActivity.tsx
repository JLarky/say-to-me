...
import { GROK_SESSION_ID } from "../external-cli/session-patterns.ts";
...
const GROK_ID = new RegExp(`^${GROK_SESSION_ID}$`, "i");
...
export function GrokActivity({
  ...
  const isGrok = Boolean(sessionId && GROK_ID.test(sessionId));
  ...
  return (
    <section {...stylex.props(card.base, queue.panel)}>
      <div {...stylex.props(styles.heading)}>
        <h2 {...stylex.props(queue.headingH2)}>Grok activity</h2>
        ...
        <div {...stylex.props(activityStyles.carousel)}>
          {items.map((item, index) => (
            <article
              ...
              aria-label={`Grok activity ${index + 1}`}
            >

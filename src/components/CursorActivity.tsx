...
import { CURSOR_SESSION_ID } from "../external-cli/session-patterns.ts";

const CURSOR_ID = new RegExp(`^${CURSOR_SESSION_ID}$`, "i");
...
export function CursorActivity({
  ...
  const isCursor = Boolean(sessionId && CURSOR_ID.test(sessionId));
  ...
  return (
    <section {...stylex.props(card.base, queue.panel)}>
      <div {...stylex.props(styles.heading)}>
        <h2 {...stylex.props(queue.headingH2)}>Cursor activity</h2>
        ...
        <div {...stylex.props(activityStyles.carousel)}>
          {items.map((item, index) => (
            <article
              ...
              aria-label={`Cursor activity ${index + 1}`}
            >
